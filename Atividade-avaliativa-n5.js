...
let orcamento = lerTeclado.question("Escolha o número do seu orçamento = 1 - Econômico 2 - Moderado 3 - Luxo: ")
console.log(`Olá, ${nomeViajante}! Vamos encontrar o destino perfeito para você.`);
let destino = "";
let resp1 = lerTeclado.questionInt("Qual tipo de clima você prefere ?  1 - Tropical/Quente, 2 - Frio/Neve, 3 - Temperado/Ameno: ")

if (resp1 === 1) {
    let resp2 = lerTeclado.questionInt("Você prefere: 1) Praia e mar 2) Floresta e ecoturismo: ")
    if (resp2 === 1) {
        let resp3 = lerTeclado.questionInt("Como você prefere sua viagem? 1) Animada — praias badaladas e muito agito 2) Tranquila — sossego e natureza preservada:")
        if (resp3 === 1) {
    ...
    let resp2 = lerTeclado.questionInt("Você quer: 1) Neve de verdade  2) Frio sem neve: ")
    if (resp2 === 1) {
        let resp3 = lerTeclado.questionInt(" Qual é o seu objetivo? 1) Praticar esportes de inverno (esqui, snowboard) 2) Contemplar a paisagem nevada e relaxar: ")
        if (resp3 === 1) {
    ...
    let resp2 = lerTeclado.questionInt("Você prefere explorar: 1) Cultura, museus e arquitetura histórico, 2) Natureza, trilhas e parques nacionais :")
    if (resp2 === 1) {
        let resp3 = lerTeclado.questionInt("1) Europa, 2) América do Sul:")
        if (resp3 === 1) {
...
let Luxo = "Dica: resorts all-inclusive e voos executivos são a sua praia!"
