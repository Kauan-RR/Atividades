...
if (modo === 1) {
    ...
    let acertou = false;
    let tentativasUsadas = 0; // Criada fora do loop para não dar erro no resumo

    console.log("Bem-vindo ao modo 1 – Eu adivinho o número!");
    console.log(`Tente adivinhar o número que estou pensando entre ${NUM_MIN} e ${NUM_MAX}. Você tem ${MAX_TENTATIVAS} tentativas.\n`);

    for (let i = 1; i <= MAX_TENTATIVAS; i++) {
        tentativasUsadas = i;
        let palpite = lerTeclado.questionInt(`Tentativa ${i} - Digite seu palpite: `);
...
// Implemente seu código abaixo:
    ...
    let min = NUM_MIN;
    let max = NUM_MAX;
    let acertouModo2 = false;
    let tentativasUsadas = 0;

    console.log("Bem-vindo ao modo 2 – Computador adivinha o número!");
    let numeroPensado = lerTeclado.questionInt(`Digite um numero inteiro entre ${NUM_MIN} e ${NUM_MAX} (prometo nao espiar na hora de jogar!): `);

    console.log(`\nLegal, gravado! Agora eu tentarei adivinhar.`);
    console.log(`Responda com: 1 – Acertei | 2 – Muito alto | 3 – Muito baixo\n`);
    ...
    for (let i = 1; i <= MAX_TENTATIVAS; i++) {
        ...
        let respostaJogador = lerTeclado.questionInt("Digite sua resposta (1/2/3): ");

        if (respostaJogador === 1) {
            console.log(`\nParabéns para mim! Acertei em ${i} tentativa(s)!`);
            acertouModo2 = true;
            break;
        } else if (respostaJogador === 2) {
            console.log("Muito alto! Vou tentar um número menor.\n");
            max = palpite - 1;
        } else if (respostaJogador === 3) {
            console.log("Muito baixo! Vou tentar um número maior.\n");
            min = palpite + 1;
        } else {
            console.log("Resposta inválida! Por favor, responda com 1, 2 ou 3.\n");
            i--;
        }
    }

    if (!acertouModo2) {
        console.log("\nPoxa, não consegui adivinhar seu número! Você venceu!");
    }

    console.log("\n--- Resumo ---");
    console.log(`Número pensado: ${numeroPensado}`);
    console.log(`Tentativas usadas: ${tentativasUsadas}`);
    console.log(`Resultado: ${acertouModo2 ? "Vitória do Computador 🏆" : "Vitória do Jogador 💀"}`);

} else if (modo === 3) {
    console.log("Saindo do jogo... Até a próxima!");
} else {
    console.log("Opção inválida! Por favor, execute o programa novamente.");
}