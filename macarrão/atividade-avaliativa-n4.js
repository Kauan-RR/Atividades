...
let anosExperiencia = ["anoExperiencia:"];
let disponivelParaFreelance = ["disponivel Para Freelance ?"];

lerTeclado.question(`${nome}`);
lerTeclado.question(`${profissao}`);
lerTeclado.question(`${cidade}`);
lerTeclado.question(`${Email}`);
lerTeclado.questionInt(`${telefone}`);
lerTeclado.questionInt(`${anosExperiencia}`);
lerTeclado.keyInYN(`${disponivelParaFreelance}, sim : não`);
...
console.log(`
 ...
 | Cidade : guaramirim |
 | E-mail : kaua32ribe29@gmail.com |
 | Telefone : 47 999083647 |
 | Experiência :  5 anos |
 | Freelance : sim |
 ============================================
`);
...
const materias = ['matematica', 'fisica', 'quimica', 'biologia', 'historia']

console.log(materias[0], materias[4])

console.log(materias.length);

console.log(`A terceira materia da lista é ${materias[3]} e a lista tem ${materias.length}  máterias no total`)
...
// O retorno -1 significa que Recife não esta no array.
console.log(cidadesBrasil.includes("Salvador"));
console.log(cidadesBrasil.includes("Belem"));
const novascidades = cidadesBrasil.concat(cidadesPortugal);
console.log(novascidades);
const cidadesDomeio = novascidades.slice(3, 7);
...
let grupos = [
    ["Batman", "Superman", "Mulher Maravilha"],
    ["baby shark", "pata branca"],
    ["Sombra Roxa", "turbo", "Mordecai", "bus"]
];
console.log(grupos);
console.log(grupos[2][1]);
 ...
 // E porque grupos so tem 3 elementos e o todosOsalunos exibe a quantidade de alunos que no caso e 9, por isso a diferença de valores, mesmo os dados sendo os mesmos.
...
console.log(produto.namo);
console.log(produto.preço);
console.log(produto.disponivel);
console.log(produto.categoria);
console.log(`O produto ${produto.nome} custa ${produto.preço} e esta disponivel: ${produto.diponivel}`);
